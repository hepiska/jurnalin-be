...
async function createSlug() {
  ...
  if (this.model.find) {
    ...
    totalData = await this.model.find({
      name: data.name
    }).countDocuments();
    ...
    if (totalData) {
      ...
      const numberOnSameSlug = await this.model.find({
        slug
      }).countDocuments();
  ...
  totalData = await this.constructor.find({
    name: this.name
  }).countDocuments();
  ...
  if (totalData) {
    ...
    const numberOnSameSlug = await this.constructor.find({
      slug: this.slug
    }).countDocuments();
